...
**ssr-demo/（TypeScript + React 18 + Express）：**

- `pnpm install` / `pnpm run dev` / `pnpm run build:client` / `pnpm run typecheck`
- 详见 [ssr-demo/CLAUDE.md](ssr-demo/CLAUDE.md)
...
**子项目隔离：** 每个目录（`AST/`、`server/`、`ssr-demo/`、`case/`、`demo/`、`utils/`）都是自包含的。依赖按子项目安装，不提升到根目录。在子项目中工作时，需 `cd` 进入对应目录并使用其自身的包管理器（大部分用 npm，ssr-demo 用 pnpm）。

**模块系统混用：**

- 根目录、`AST/`、`server/` — CommonJS（`require`/`module.exports`）
- `ssr-demo/` — ESM（`import`/`export`，`"type": "module"`）

**无测试框架** — 根目录的 `test` 脚本是占位符。`demo/test-demo/` 包含独立的 JS 文件，用 `node` 单独运行以学习 JS 概念（非自动化测试）。

**重点子项目：ssr-demo/** — 最完整的项目。手写 SSR + React 18 hydration + esbuild 打包 + Express 服务。拥有独立的 CLAUDE.md 和 AGENTS.md，包含详细架构和踩坑记录。
...
- 根目录的 lock 文件被 gitignore；`ssr-demo/` 提交了 `pnpm-lock.yaml`
